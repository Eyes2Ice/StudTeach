...
@keyframes pageAnim" +
  ...
  " +
      p1 +
      "% { transform: rotateZ(0deg) translateX(-18px); -webkit-transform: rotateZ(0deg) translateX(-18px); } \
  " +
      p2 +
      "%, " +
      p3 +
      "% { transform: rotateZ(180deg) translateX(-18px); -webkit-transform: rotateZ(180deg) translateX(-18px); } \
  " +
      p4 +
      "% { transform: rotateZ(0deg) translateX(-18px); -webkit-transform: rotateZ(0deg) translateX(-18px); } \
}\
@-webkit-keyframes pageAnim" +
  ...
  function buildPages() {
    ...
    for (var i = 0; i < PAGES; i++) {
      var li = document.createElement("li");
      li.style.animationName = "pageAnim" + i;
      li.style.webkitAnimationName = "pageAnim" + i; // Добавляем префикс для Safari
      li.style.animationDuration = DURATION + "s";
      li.style.webkitAnimationDuration = DURATION + "s"; // Префикс для Safari
      li.style.animationTimingFunction = "ease";
      li.style.webkitAnimationTimingFunction = "ease"; // Префикс для Safari
      li.style.animationIterationCount = "infinite";
      li.style.webkitAnimationIterationCount = "infinite"; // Префикс для Safari
      ul.appendChild(li);
    }
  }
